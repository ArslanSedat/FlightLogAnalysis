...
$$
q = \begin{bmatrix} q_{e0} & q_{ex} & q_{ey} & q_{ez} \end{bmatrix}
$$
 
$$
\begin{bmatrix} \psi \\ \theta \\ \phi \end{bmatrix} = \text{quat2eul}(q, \ 'ZYX')
$$
...
Lift ($C_L$), Drag ($C_D$), and Side Force ($C_Y$) coefficients are calculated as:
...
$$
\rho = f(altitude) = \text{atmoscoesa}(alt_m)
$$
...
$$
\begin{aligned}
m &\to ft = m \times 3.28084 \\
m/s &\to kts = m/s \times 1.94384 \\
rad &\to deg = rad \times \frac{180}{\pi}
\end{aligned}
$$
